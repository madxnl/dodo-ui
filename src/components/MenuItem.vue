...
.DropdownItem:hover {
  background: color-mix(in lab, var(--dodo-color-text) 3%, transparent);
}
.DropdownItem.active {
  background: color-mix(in lab, var(--dodo-color-info) 15%, transparent);
}
.emphasize .text {
...
.separator {
  ...
  border: 0;
  border-top: 1px solid color-mix(in lab, var(--dodo-color-text) 10%, transparent);
}
</style>
